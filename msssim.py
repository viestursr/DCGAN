...
# Adapted and modified from implementation in PGGAN repository
# Source: https://github.com/tkarras/progressive_growing_of_gans/blob/master/metrics/ms_ssim.py
...
image_files = list(glob.glob("msssim/celeba/train/*"))
...
with tf.Session() as sess:
    ...
    decoded_image = tf.image.rgb_to_grayscale(tf.expand_dims(tf.image.decode_png(input_img, channels=3), 0))

    for i in tqdm(range(0, len(image_files))):
        with open(image_files[i], 'rb') as image_file:
            img_str = image_file.read()

        dec = sess.run(decoded_image, feed_dict={input_img: img_str})

        if len(images) == 0:
            images = np.array(dec)
        else:
            images = np.concatenate((images, np.array(dec)), 0)
